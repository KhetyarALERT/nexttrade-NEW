...
// OKX Admin Control Hub - Manage sub-account pool, assignments, withdrawals
// v1.0.0 - Manual sub-account management system
  ...
  try {
    ...
    if (action === 'checkPoolBalance') {
      ...
      if (!balanceResult.ok) {
        await base44.asServiceRole.entities.OKXSubAccountPool.update(poolAccountId, {
          status: 'ERROR',
          notes: `Balance check failed: ${balanceResult.error?.okxMsg || 'Unknown'}`,
      ...
      const details = balanceResult.data?.data?.[0]?.details || [];
      ...
      const usdtEntry = balances.find(b => b.currency === 'USDT');
      const nowISO = new Date().toISOString();

      await base44.asServiceRole.entities.OKXSubAccountPool.update(poolAccountId, {
        last_balance_check: nowISO,
        last_balance_usdt: usdtEntry?.total || 0,
    ...
    if (action === 'adminTransfer') {
      ...
      const transferBody = {
        ccy: currency,
        amt: String(amount),
        from: direction === 'toSub' ? '6' : '6', // 6 = funding account
        to: direction === 'toSub' ? '6' : '6',
      ...
      const status = transferResult.ok ? 'COMPLETED' : 'FAILED';
      ...
      auditLog('ADMIN_TRANSFER', user.id, { direction, poolAccountId, currency, amount, status });
    ...
    if (action === 'listUsers') {
      ...
      const users = await base44.asServiceRole.entities.User.filter({}, '-created_date', limit);
