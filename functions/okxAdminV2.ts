...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('[OKX_ADMIN_V2] Action:', action, 'User:', user.email, 'Role:', user.role);
    ...
    if (action === 'getTransactionHistory') {
      ...
      const { poolAccountId, limit = 50 } = params;
      if (!poolAccountId) {
        return Response.json({ ok: false, error: { code: 'MISSING_FIELDS', message: 'poolAccountId required' } }, { status: 400 });
      }
      ...
      const [depositResult, withdrawResult, billsResult] = await Promise.all([
        okxRequest({ credential, method: 'GET', path: '/api/v5/asset/deposit-history', query: { limit: String(limit) }, isTradingEndpoint: false }),
        okxRequest({ credential, method: 'GET', path: '/api/v5/asset/withdrawal-history', query: { limit: String(limit) }, isTradingEndpoint: false }),
        okxRequest({ credential, method: 'GET', path: '/api/v5/account/bills', query: { limit: String(limit) }, isTradingEndpoint: true }),
      ]);

      return Response.json({
        ok: true,
        data: {
          deposits: (depositResult.data?.data || []).map(d => ({
            txId: d.txId,
            chain: d.chain,
            currency: d.ccy,
          ...
          withdrawals: (withdrawResult.data?.data || []).map(w => ({
            wdId: w.wdId,
            txId: w.txId,
            chain: w.chain,
            currency: w.ccy,
          ...
          bills: (billsResult.data?.data || []).map(b => ({
            billId: b.billId,
            instId: b.instId,
            subType: b.subType,
            currency: b.ccy,
    ...
    console.error('[OKX_ADMIN_V2_ERROR]', error.message);
