...
Deno.serve(async (req) => {
  ...
  const fundingBal = parseFloat(fundRes.data?.data?.[0]?.availBal || '0');
  
  console.log(`Current Funding Balance: ${fundingBal}`);
  
  if (fundingBal < 500) {
      return Response.json({ error: "Insufficient funding balance", balance: fundingBal });
  }

  // Execute Sweep: 500 from Sub-Funding -> Master
  const masterCreds = getMasterCredentials();
  // type '2' = master account from sub account
  const sweepRes = await okxRequest({
      ...
      body: { 
          ...
          subAcct: targetAccount.external_account_id 
      }
  });

  if (!sweepRes.ok) {
      return Response.json({ error: "Sweep Failed", details: sweepRes.error });
  }

  return Response.json({
    success: true,
    message: "Swept 500 USDT from Sub-Funding to Master.",
    transId: sweepRes.data?.data?.[0]?.transId,
    previousBalance: fundingBal,
    newBalance: fundingBal - 500
  });
});