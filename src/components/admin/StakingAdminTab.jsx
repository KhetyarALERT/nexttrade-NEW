      ...
      <Tabs value={adminTab} onValueChange={setAdminTab} className="space-y-4">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b pb-4">
          <TabsList className="bg-muted/50 p-1">
            ...
            <TabsTrigger value="payouts" className="px-4" onClick={() => { if (!payoutSummary) loadPayoutSummary(); }}>Payouts {payoutSummary?.pendingPayoutsCount > 0 && <span className="ml-2 bg-emerald-500 text-white text-[10px] px-1.5 py-0.5 rounded-full">{payoutSummary.pendingPayoutsCount}</span>}</TabsTrigger>
