...
export default function SuggestionChips({ 
  ...
  const getIcon = (type) => {
    switch (type) {
      ...
      case "prompt": 
        return suggestions.find(s => s.key === "talk_human") ? MessageCircle : HelpCircle;
  ...
  return (
    <div className={cn("flex flex-wrap gap-2 py-2", isRtl && "justify-end")}>
      {suggestions.map((suggestion) => {
        ...
        const label = language === "ar" ? suggestion.label_ar : suggestion.label_en;
        const isTalkHuman = suggestion.key === "talk_human";
...
SuggestionChips.propTypes = {
  suggestions: PropTypes.arrayOf(PropTypes.shape({
    ...
    type: PropTypes.oneOf(["prompt", "route", "video"]).isRequired,
