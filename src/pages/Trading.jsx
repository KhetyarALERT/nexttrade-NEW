...
export default function Trading({ language = "en" }) {
  ...
  useEffect(() => {
    if (dailyOpen > 0 && currentPrice > 0) {
      setPriceChange(((currentPrice - dailyOpen) / dailyOpen) * 100);
    }
  }, [currentPrice, dailyOpen]);
      ...
      <header className="h-14 bg-[#1a1a2e] border-b border-slate-700/50 px-4 flex items-center justify-between shrink-0 z-20">
              ...
              <Button variant="ghost" className="h-10 px-3 text-white hover:bg-slate-700/30 gap-3 rounded-lg border border-transparent hover:border-slate-700/50 transition-all">
                ...
                <div className="flex flex-col items-start">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant="outline" className="bg-blue-600/10 text-blue-400 border-blue-500/30 text-[10px] px-1.5 py-0 h-4 uppercase tracking-wider">
                      {t.perpetual}
                    </Badge>
                  </div>
                </div>
                  ...
                  return (
                    <DropdownMenuItem
                      ...
                      className={`flex items-center justify-between p-3 cursor-pointer transition-colors ${
                        isSelected ? "bg-blue-600/10 border-l-2 border-blue-500" : "hover:bg-slate-700/30 border-l-2 border-transparent"
                      }`}
                    >
          ...
          <div className="flex items-center gap-4">
            <div className="flex flex-col">
              <span className="text-white text-lg font-bold font-mono leading-none">
                ${currentPrice.toLocaleString(undefined, { minimumFractionDigits: currentPrice < 1 ? 6 : 2, maximumFractionDigits: currentPrice < 1 ? 6 : 2 })}
              </span>
        ...
        <div className="flex items-center gap-6">
          ...
          <Button
            ...
            className="h-9 border-slate-700 bg-slate-800/30 text-slate-300 hover:bg-blue-600 hover:text-white hover:border-blue-500 transition-all rounded-lg px-4"
          >
