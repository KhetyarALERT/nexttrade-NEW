import type { PairDetails } from './types';

export const selectBestPair = (pairs: PairDetails[]): PairDetails | null => {
...
export const formatAge = (timestamp?: number | null) => {
  if (!timestamp) return '—';
...
export const formatTokenAmount = (amount: string | number | null | undefined, decimals = 6) => {
  if (!amount) return '0';
